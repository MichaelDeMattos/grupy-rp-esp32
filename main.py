...
def login(host, api, user, password):
	try:
		...
		req = requests.post(url=host + api, headers = {'content-type': 'application/json'}, data=data_post)
...
def handler_iot(host, api, token, params):
	try:
		req = requests.get(
			url=host + api + params, headers={"content-type": "application/json", "Authorization": token})
		if req.status_code != 200:
			return ""
